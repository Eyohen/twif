...
export default function StoreManagerOverviewPage({ sentInvoices = [], productionJobs = [], onNavigate }) {
  ...
  const kpis = [
    ...
    { Icon: TrendingUp, label: 'Awaiting Payment', value: awaitingPayment.length, detail: 'View and follow up', tone: 'gold', dest: 'Invoices' },
    { Icon: Package, label: 'Ready for Collection', value: readyJobs.length, detail: 'View collections', tone: 'green', dest: 'Orders' },
    { Icon: Users, label: 'Customers Today', value: today, detail: '+3 new customers', tone: 'purple', dest: 'Customers' },
  ];

  const pendingItems = [
    { Icon: AlertCircle, label: 'Awaiting Measurements', detail: `${missingMeasurements.length} customers`, tone: 'orange', dest: 'Customers' },
    { Icon: TrendingUp, label: 'Awaiting Payment', detail: `${awaitingPayment.length} invoices`, tone: 'gold', dest: 'Invoices' },
    { Icon: FileText, label: 'Waiting for Accounts Approval', detail: `${pendingApproval.length} invoices`, tone: 'purple', dest: 'Invoices' },
  ...
  return (
    <div className="store-overview">
      ...
      <section className="store-overview-kpis">
        {kpis.map(({ Icon, label, value, detail, tone, dest }) => (
          <article
            className={tone}
            key={label}
      ...
      <section className="store-overview-main-grid">
        ...
        <article className="store-overview-panel pending-actions">
          ...
          <div>
            {pendingItems.map(({ Icon, label, detail, tone, dest }) => (
              <section
                key={label}
                style={{ cursor: onNavigate ? 'pointer' : 'default' }}
                onClick={() => onNavigate?.(dest)}
